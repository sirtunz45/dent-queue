...
function Queue() {
  ...
  const handleConfirmBookingPopup = async () => {
    ...
    try {
      const defaultPregnancyStatus = "ไม่ระบุ"; // สามารถเปลี่ยนเป็น "ไม่ตั้งครรภ์" ได้
      const defaultLastPeriodDate = null; // ให้เป็น null หากไม่มีข้อมูล
      ...
      const response = await axios.post("/api/db", {
        ...
        time_state,
        treatmentRight,
        isPregnant: defaultPregnancyStatus,
        lastPeriodDate: defaultLastPeriodDate
      });
