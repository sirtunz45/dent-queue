...
await connection.execute(query, values);
    ...
    const { id } = req.query;
    const { status, time_state } = req.body;

    console.log("Received data in API:", { id, status, time_state }); // ✅ ตรวจสอบค่าที่ได้รับ
    ...
    try {
      ...
      console.log("Executing SQL Query:", query, [status, time_state, id]); // ✅ ตรวจสอบคำสั่ง SQL ที่กำลังรัน

      const [result] = await connection.execute(query, [status, time_state, id]);

      console.log("Database Update Result:", result); // ✅ ตรวจสอบผลลัพธ์จากฐานข้อมูล

      return res.status(200).json({ message: 'Status and time_state updated successfully!' });
    } catch (error) {
      console.error('Database Error:', error.message);
      res.status(500).json({ error: 'Failed to update status and time_state.' });
    } finally {
      if (connection) {
        await connection.end();
      }
