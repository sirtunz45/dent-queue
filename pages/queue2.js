...
function Queue() {
  ...
  const [unavailableTimes, setUnavailableTimes] = useState([]); // เวลาที่ไม่สามารถเลือกได้
  ...
  const selectDate = (day) => {
    const formattedDate = `${year - 543}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    setSelectedDate(`${day} ${months[month]} ${year}`);
    fetchUnavailableTimes(formattedDate); // ดึงเวลาที่ไม่สามารถจองได้
  };
  ...
  const fetchUnavailableTimes = async (date) => {
    try {
      const response = await axios.get(`/api/db?date=${date}`);
      if (response.status === 200) {
        setUnavailableTimes(response.data.unavailableTimes); // รับข้อมูลเวลาที่ถูกจองแล้ว
      }
    } catch (error) {
...
<div style={{ textAlign: 'center', marginTop: '20px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', alignItems: 'center', justifyContent: 'center' }}>
  {Object.keys(timeStateMapping).map((time) => {
    const isUnavailable = unavailableTimes.includes(timeStateMapping[time]); // ตรวจสอบเวลาที่ไม่ว่าง
    ...
    return (
      <button
        ...
        disabled={isUnavailable} // ล็อคปุ่มที่ไม่ว่าง
        style={{
          padding: '10px 20px',
          margin: '5px',
          borderRadius: '5px',
          ...
          backgroundColor: isUnavailable
            ? '#f8d7da' // สีชมพูอ่อนสำหรับเวลาที่ไม่ว่าง
            : isSelected
            ? '#e676c5' // สีชมพูเข้มสำหรับเวลาที่เลือก
            : '#fe1d93', // สีปกติ
          color: isUnavailable ? '#666' : '#fff', // สีข้อความ
